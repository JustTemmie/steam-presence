...
def get_coverart_link(config: Config, art_hash: str) -> Optional[str]:
    ...
    def upload_task():
        ...
        if config.mpd.catbox:
            ...
            if catbox_link:
                disk_cache.cache_store(bank="mpd", key=art_hash, value={"url": catbox_link}, ttl=72*3600)
                return
        
        if config.mpd.uguu:
            uguu_link = upload_file.upload_uguu_file(file_path)
            if uguu_link:
                disk_cache.cache_store(bank="mpd", key=art_hash, value={"url": uguu_link}, ttl=3*3600)
                return
