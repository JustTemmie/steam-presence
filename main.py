...
try:
    ...
    # used to connect API Discord (Gateway)
    import websocket
    import threading

except:
    answer = input(
        "looks like either requests, pypresence, steamgrid, psutil, websocket-client, or beautifulSoup is not installed, do you want to install them? (y/n) ")
    if answer.lower() == "y":
...
# Initialize global variable to avoid reference errors in classes
gameName = ""


# Class to connect directly to Discord API (Gateway)
class GatewayPresence:
    ...
    def on_open(self, ws):
        self.connected = True
        # Send Identification Payload (Opcode 2)
        auth_payload = {
            "op": 2,
            "d": {
                "token": self.token,
                "properties": {
                    "$os": "linux",
                    "$browser": "steam-presence",
                    "$device": "steam-presence"
                },
                "presence": {
                    "status": "online",
                    "afk": False
    ...
    def update(self, details=None, state=None, start=None, large_image=None, large_text=None, small_image=None,
               small_text=None, buttons=None):
        if not self.connected or not self.ws:
            return

        # Access the global gameName variable to ensure we have the correct name
        global gameName

        # Build the activity
        activity = {
            "application_id": str(self.client_id),
            "name": gameName if gameName else "Game",
            "type": 0,
            "timestamps": {}
        }
        ...
        # FIX: Multiply by 1000 because Discord Gateway uses milliseconds
        if start: activity["timestamps"]["start"] = int(start * 1000)

        assets = {}
        # FILTER: Gateway does not allow external URLs in assets for users.
        if large_image and not str(large_image).startswith("http"):
        ...
        try:
            ...
            print(f"Failed to update presence via Gateway: {e}")
...
def error(message):
    print(f"    ERROR: [{datetime.now().strftime('%b %d %Y - %H:%M:%S')}] {message}")
...
def getImageFromSGDB(loops=0):
    ...
    if gridIcons != None:
        ...
        for i, gridIcon in enumerate(gridIcons):
            gridIconsDict[i] = [gridIcon.height, gridIcon._nsfw, gridIcon.url, gridIcon.mime, gridIcon.author.name,
                                gridIcon.id]
...
def getGameSteamID():
    ...
    if r.status_code != 200:
        error(
            f"error code {r.status_code} met when requesting list of games in order to obtain an icon for {gameName}, ignoring")
        return
...
def getSteamPresence():
    r = makeWebRequest(
        f"https://api.steampowered.com/ISteamUser/GetPlayerSummaries/v2/?key={steamAPIKey}&format=json&steamids={userID}")
...
def getSteamRichPresence():
    for i in userID.split(","):
        ...
        if pageRequest.status_code != 200:
            error(
                f"status code {pageRequest.status_code} returned whilst trying to fetch the enhanced rich presence info for steam user ID `{i}`, ignoring function")
            return
...
def getImageFromDiscord():
    ...
    try:
        ...
        if r.status_code != 200:
            error(
                f"status code {r.status_code} returned when requesting more info about {gameName} from Discord, ignoring")
            coverImage = None
            coverImageText = None
            return
...
def verifyProjectVersion():
    ...
    if metaFile["structure-version"] == "0":
        ...
        try:
            ...
            error(
                f"error encountered whilst trying to update the config-version to version 1, exiting\nError encountered: {e}")
            exit()
        print("----------------------------------------------------------")
...
def checkForUpdate():
    ...
    parsableCurrentVersion = currentVersion.replace("v",
                                                    "")  # the `currentVersion` variable is set in the main() function so i'm less likely to forget, lol
    ...
    for new, old in zip(parsableNewestVersion, parsableCurrentVersion):
        if int(new) > int(old):
            ...
            print(
                f"if you wish to upload from `{currentVersion}` to `{newestVersion}` simply run `git pull` from the terminal/cmd in the same folder as main.py")
            print(
                f"commits made in this time frame: https://github.com/JustTemmie/steam-presence/compare/{currentVersion}...{newestVersion}")
            print("----------------------------------------------------------")
            # untested, but it should work
            if int(parsableNewestVersion[0]) == 2:
                print(
                    "DO NOTE, UPDATING TO VERSION 2 OF STEAM PRESENCE WILL REQUIRE YOU TO COMPLETELY REDO YOUR CONFIG FILE")
                print("----------------------------------------------------------")
            return
...
def main():
    ...
    using_gateway = False
    if discordToken and discordToken != "DISCORD_TOKEN" and discordToken != "":
        log("DISCORD_TOKEN found: Using direct Gateway connection instead of local IPC.")
        using_gateway = True
    else:
        log("No DISCORD_TOKEN found: Using local Discord IPC (Discord client must be running).")
    ...
    while True:
        ...
        steamAPIKey = config["STEAM_API_KEY"]
        discordToken = config.get("DISCORD_TOKEN", "")  # Recargar token también
        defaultAppID = config["DISCORD_APPLICATION_ID"]
        defaultLocalAppID = config["LOCAL_GAMES"]["LOCAL_DISCORD_APPLICATION_ID"]
        doLocalGames = config["LOCAL_GAMES"]["ENABLED"]
        localGames = config["LOCAL_GAMES"]["GAMES"]
        ...
        if previousGameName != gameName:
            ...
            # if the game has changed or a new game has been opened
                ...
                if using_gateway:
                    RPC = GatewayPresence(discordToken, appID)
                else:
